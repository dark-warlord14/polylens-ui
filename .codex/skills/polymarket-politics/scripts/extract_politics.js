#!/usr/bin/env node
// Usage: node extract_politics.js [--days N] [--min-vol N] [--now ISO]
// Reads ./src/data/cache.json and prints top 30 political market candidates as JSON lines.
...
const MAX_DAYS = Math.min(parseInt(get('--days', '7'), 10), 14);
...
// Categories derived from API event tags — no keyword/slug patching needed.
...
data.deals
  ...
  .filter(d => d.daysLeft >= 0 && d.daysLeft <= MAX_DAYS && POLITICS_CATS.has(d.category))
  .forEach(d => {
